...
function BeamPanel({
  ...
  return (
    <div className={`flex flex-col min-h-0 ${highlight ? "bg-primary/[0.03]" : ""}`}>
      <div className="px-2 py-1 border-b border-border/40 bg-secondary/15 shrink-0">
        <div className="min-w-0 flex items-baseline gap-2">
          <span className="mono text-[10px] font-bold uppercase tracking-wider text-foreground whitespace-nowrap truncate">
            {title}
          </span>
          <span className="mono text-[9px] text-foreground/70 shrink-0">({displayCount})</span>
        </div>
      </div>
      {beams.length === 0 ? (
        <p className="px-2 py-1.5 mono text-[11px] text-foreground/70 flex-1">{emptyMessage}</p>
      ) : (
      ...
      {footerAction && (
        <div className="px-2 py-1.5 border-t border-border/40 bg-secondary/10 shrink-0 mt-auto">
          {footerAction}
        </div>
      )}
    </div>
