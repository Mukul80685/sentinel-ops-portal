...
const SUPPORT_MODULES = [
  {
    ...
    icon: Radar,
    desc: "EIRP visibility matrix — coverage windows and signal-strength data.",
  },
  {
    to: "/inventory",
    title: "Resource Inventory",
    icon: Boxes,
    desc: "Equipment catalogue — antennas, LNAs, demodulators and processors.",
  },
  {
    to: "/serviceability",
    title: "Serviceability State",
    icon: Shield,
    desc: "Equipment readiness across all operational units.",
  },
] as const;
  ...
  return (
    <AppShell isHome>
      <div className="flex flex-col gap-3 h-full min-h-0">
        ...
        <div className="shrink-0 grid grid-cols-4 rounded-md border-2 border-primary/45 overflow-hidden bg-card min-h-[132px]">
          ...
          <div className="col-span-1 flex flex-col justify-center gap-2 px-3 py-3 border-r border-border/80">
            {CC_GROUP_A.map(({ id, title, icon: Icon }) => (
              ...
              >
                ...
                <ChevronRight className="h-3 w-3 text-foreground opacity-0 group-hover/btn:opacity-100 transition-opacity" />
              </Link>
            ))}
          </div>
          ...
          <div className="col-span-1 flex items-center justify-center px-3 py-3 bg-secondary/15 min-h-[80px]">
            <p className="mono text-[9px] sm:text-[10px] text-foreground/80 leading-snug text-center line-clamp-3 font-normal normal-case tracking-normal">
              Operational Command Hub for Live Intelligence &amp; Resource Control
            </p>
          </div>
        </div>

        {/* ── Secondary modules — 3 equal columns, no scroll ─────────────── */}
          ...
          {SUPPORT_MODULES.map((mod) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div className="mono text-[10px] sm:text-[11px] font-bold uppercase tracking-tight text-foreground leading-snug">
                  {mod.title}
                </div>
              </div>
              <p className="text-[10px] sm:text-[11px] text-foreground/90 leading-snug flex-1 line-clamp-2 font-medium">
                {mod.desc}
              </p>
