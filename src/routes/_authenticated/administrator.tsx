...
export function OptimizationEngine() {
  ...
  if (selectedUnit && selectedOpt) {
    return (
      <UnitDetailView
        data={selectedOpt}
        onBack={() => navigate({ to: ".", search: (prev: Record<string, unknown>) => ({ ...prev, unit: undefined }) })}
      />
    );
  }
            ...
            {(tableExpanded ? rows : rows.slice(0, 4)).map(({ state, data: d }, idx) => {
              ...
              return (
                <tr key={state.unitDbId}
                  onClick={() => navigate({ to: ".", search: (prev: Record<string, unknown>) => ({ ...prev, unit: navKey }) })}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ADMINISTRATOR — Operational subsystem launch + embedded module views
// ═══════════════════════════════════════════════════════════════════════════════
...
// DECISION: 3+2 brick layout — row 2 tiles sit in the gaps between row 1 (6-column grid).
// Icons/titles come from ADMIN_HUB_STANDALONE (controlCenter.ts) — never reference lucide icons here.
